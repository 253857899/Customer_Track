...
@Controller
@RequestMapping("Customer_pool_finish")
public class Customer_pool_finishController extends BasicController<Customer>{
	...
	public ReturnInfo index(String txt, Integer page, Integer limit) {
		String where=" where  c_client.execstatus = '2' ";
		if(txt!=null && txt!="")
			where=" where  c_client.operatorids = '2' and c_client.name like '%"+txt+"%'";
