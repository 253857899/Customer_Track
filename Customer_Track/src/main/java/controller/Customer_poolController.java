...
@Controller
@RequestMapping("Customer_pool")
public class Customer_poolController extends BasicController<Customer>{
	...
	public ReturnInfo index(String txt, Integer page, Integer limit) {
		String where=" where c_client.operatorids is null or c_client.operatorids = '' ";
		if(txt!=null && txt!="")
			where=" where (c_client.operatorids is null or c_client.operatorids = '') and c_client.name like '%"+txt+"%'";
